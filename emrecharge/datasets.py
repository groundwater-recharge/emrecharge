...
class EMDataset:
    ...
    def __init__(self, csv_filename: str, csv_filename_thickness: str, spatial_unit='m'):
        ...
        if spatial_unit == 'ft':
            self.spatial_conversion = 1./0.3048
        elif spatial_unit == 'm':
            self.spatial_conversion = 1.
    ...
    @property
    def topography(self):
        return self.df[["UTMX", "UTMY", "ELEVATION"]].values[:, :] * self.spatial_conversion
    ...
    def lines_xy(self):
        ...
        for g, data in self.df[["LINE_NO", "RECORD", "UTMX", "UTMY"]].groupby(
            ...
            lines.append(g)
            em_lines[g] = data[["UTMX", "UTMY", "RECORD"]].values*self.spatial_conversion
    ...
    def get_binned_resistivity_by_line(self, line_number: int, n_bins: int, maximum_depth: float):
        ...
        f_doi = interp1d(distance_bins_center[~np.isnan(doi_bins)], doi_bins[~np.isnan(doi_bins)], fill_value='extrapolate')
        doi_bins_filled = f_doi(distance_bins_center)
        f_elevation = interp1d(distance_bins_center[~np.isnan(elevation_bins)], elevation_bins[~np.isnan(elevation_bins)], fill_value='extrapolate')
        elevation_bins_filled = f_elevation(distance_bins_center)
        depth = np.cumsum(np.r_[0., self.hz[:n_layer]])
        return distance, distance_bins, depth, rho_bins, elevation_bins_filled, doi_bins_filled